...
email_re = re.compile(r'^(\w|\.|_|-)+[@](\w|_|-|\.)+[.]\w{2,3}$')
...
def register(user: schemas.UserCreate,
    ...
    if not smtp.verify_email(user.email):
        raise HTTPException(status_code=400,
...
def send_token(email: str, db: Session = Depends(database.get_db)):
    ...
    """
    # Generate token
    user, token = users.generate_token(db, email=email)
    if token is not None and email != "":
        # Send it
        smtp.send_token(user, token)
    # Hide problems
    return {"ok": True}
...
def permit_download_request(
    ...
    if user is not None and permit_bool:
        smtp.send_picture(request, user)
    # Delete download request from DB
    users.download_status(db, request=request, permit=permit_bool)
    return {"ok": True}
